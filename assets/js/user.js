const userState = { user: null, finance: null, plans: [], recommendations: [], active: "dashboard", sort: { key: "user_plan_name", dir: 1 } };
...
function projectedProfit(plan) {
  const months = plan.duration_months || (plan.horizon === "Long" ? 60 : plan.horizon === "Medium" ? 36 : 12);
  return projectedTotalValue(plan) - (Number(plan.investment_amount || 0) + (Number(plan.monthly_amount || 0) * months));
}

function expectedMonthlyGrowth(plan) {
  const months = plan.duration_months || (plan.horizon === "Long" ? 60 : plan.horizon === "Medium" ? 36 : 12);
  if (!months) return 0;
  return projectedProfit(plan) / months;
}
...
function calculator() {
  ...
  $("#calculator-form").addEventListener("submit", async (event) => {
    ...
    $("#recommendations").innerHTML = `<article class="recommendation-card"><header><strong>${escapeHtml(item.bank_name)}</strong><span class="badge good">Score ${item.score}/8</span></header>
      <div class="metric-list"><div><span>Plan Name</span>${escapeHtml(item.user_plan_name)}</div><div><span>Goal</span>${escapeHtml(input.investment_goal)}</div><div><span>Expected Return</span>${item.expected_return}%</div><div><span>Risk Level</span>${escapeHtml(item.risk)}</div><div><span>Liquidity</span>${escapeHtml(item.liquidity)}</div><div><span>Suggested Horizon</span>${escapeHtml(item.horizon)}-term</div><div><span>Monthly Contribution</span>${item.allows_monthly ? "Yes" : "No"}</div><div><span>Estimated Total Value</span>${money(item.estimated_total_value)}</div><div><span>Estimated Profit</span>${money(item.estimated_profit)}</div><div><span>Expected Monthly Growth</span>${money(item.estimated_monthly_growth)}</div></div>
      <p class="muted">${escapeHtml(item.bank_contact)}<br>${escapeHtml(item.bank_details || "")}</p><div class="actions"><button class="primary" id="save-rec">Save Plan</button><a class="secondary button-link" href="${escapeHtml(item.bank_website || "#")}" target="_blank" rel="noreferrer">Investment Page</a></div></article>`;
    $("#save-rec").addEventListener("click", async () => {
      await api("plans", { method: "POST", body: { bank_id: item.bank_id, user_plan_name: input.plan_name, investment_amount: input.investment_amount, monthly_contribution: Boolean(input.monthly_contribution), monthly_amount: input.monthly_amount || 0, investment_goal: input.investment_goal, risk: item.risk, liquidity: item.liquidity, horizon: item.horizon, expected_return: item.expected_return, score: item.score, bank_investment_url: item.bank_website || "#" } });
...
function planTable(editable) {
  const plans = [...userState.plans].sort((left, right) => {
    const valueFor = (plan) => {
      if (userState.sort.key === "projected_total") return projectedTotalValue(plan);
      return plan?.[userState.sort.key] ?? "";
    };
    const a = valueFor(left);
    const b = valueFor(right);
    return String(a).localeCompare(String(b), undefined, { numeric: true, sensitivity: "base" }) * userState.sort.dir;
  });
  return `<div class="table-wrap"><div class="table-tools"><input data-search-table placeholder="Search plan, bank, goal, or risk"></div><table><thead><tr><th>${planSortHeader("user_plan_name", "Plan Name")}</th><th>${planSortHeader("investment_goal", "Goal")}</th><th>${planSortHeader("investment_amount", "Amount")}</th><th>${planSortHeader("projected_total", "Projected Total")}</th><th>Monthly Growth</th><th>${planSortHeader("risk", "Risk")}</th><th>${planSortHeader("horizon", "Horizon")}</th><th>${planSortHeader("liquidity", "Liquidity")}</th><th>Monthly</th><th>${planSortHeader("bank_name", "Bank")}</th><th>Investment Link</th><th>Actions</th></tr></thead><tbody>${plans.map((plan) => `<tr><td>${escapeHtml(plan.user_plan_name)}</td><td>${escapeHtml(plan.investment_goal || "N/A")}</td><td>${money(plan.investment_amount)}</td><td>${money(projectedTotalValue(plan))}</td><td>${money(expectedMonthlyGrowth(plan))}</td><td><span class="pill risk-${String(plan.risk).toLowerCase()}">${escapeHtml(plan.risk)}</span></td><td>${escapeHtml(plan.horizon)}-term</td><td><span class="pill liquidity-${String(plan.liquidity).toLowerCase()}">${escapeHtml(plan.liquidity)}</span></td><td>${Number(plan.monthly_contribution) ? `Yes (${money(plan.monthly_amount)})` : "No"}</td><td>${escapeHtml(plan.bank_name || "N/A")}<br><span class="muted">${escapeHtml(plan.bank_contact || "")}</span></td><td>${plan.bank_website ? `<a class="secondary button-link" href="${escapeHtml(plan.bank_website)}" target="_blank" rel="noreferrer">Open Page</a>` : `<span class="muted">N/A</span>`}</td><td class="actions"><button class="secondary" data-view-plan="${plan.id}">View</button>${editable ? `<button class="secondary" data-edit-plan="${plan.id}">Edit</button><button class="danger" data-delete-plan="${plan.id}">Delete</button>` : ""}</td></tr>`).join("") || `<tr><td colspan="12" class="table-empty">No plans saved yet.</td></tr>`}</tbody></table></div>`;
}
...
function wirePlans() {
  ...
  document.querySelectorAll("[data-plan-sort]").forEach((button) => button.addEventListener("click", () => {
    ...
    userState.sort = { key, dir: userState.sort.key === key ? userState.sort.dir * -1 : 1 };
    userState.active === "plans" ? plans() : report();
  }));
...
function planSortHeader(key, label) {
  const arrow = userState.sort.key === key ? (userState.sort.dir === 1 ? "↑" : "↓") : "↕";
  return `<button class="sort-btn" data-plan-sort="${key}">${label}<span class="sort-arrow">${arrow}</span></button>`;
}

function report() {
  const finance = userState.finance || {};
  const projectedPortfolioValue = userState.plans.reduce((sum, plan) => sum + projectedTotalValue(plan), 0);
  const projectedMonthlyGrowthTotal = userState.plans.reduce((sum, plan) => sum + expectedMonthlyGrowth(plan), 0);
  const goals = [...new Set(userState.plans.map((plan) => (plan.investment_goal || "").trim()).filter(Boolean))];
  $("#content").innerHTML = `<div class="section-head"><div></div><button class="primary" id="download-report">Download PDF</button><button class="secondary" id="print-report">Print Report</button></div>
    ${cards([{ label: "User", value: `${escapeHtml(userState.user.full_name)} ${escapeHtml(userState.user.surname)}` }, { label: "Net Salary", value: money(finance.net_salary) }, { label: "Savings", value: money(finance.current_savings) }, { label: "Projected Total", value: money(projectedPortfolioValue) }, { label: "Monthly Growth", value: money(projectedMonthlyGrowthTotal) }, { label: "Goals", value: goals.length || 0 }])}
    <div class="grid-2"><section class="panel"><h2>Risk Summary</h2><div class="chart-box"><canvas id="risk-chart"></canvas></div></section><section class="panel"><h2>Financial Overview</h2><p>Gross salary: ${money(finance.gross_salary)}</p><p>Deductions: ${money(finance.deductions)}</p><p>Expenses: ${money(finance.monthly_expenses)}</p><p>Current savings: ${money(finance.current_savings)}</p></section></div>
    <section class="panel" style="margin-top:18px"><h2>Goals</h2>${goals.length ? `<ul class="goal-list">${goals.map((goal) => `<li>${escapeHtml(goal)}</li>`).join("")}</ul>` : `<p class="muted">No goals saved yet.</p>`}</section>
  ...
  $("#download-report").addEventListener("click", () => downloadUserPdf({ finance, projectedPortfolioValue, projectedMonthlyGrowthTotal, goals }));
}

function downloadUserPdf({ finance, projectedPortfolioValue, projectedMonthlyGrowthTotal, goals }) {
  if (!window.jspdf) {
    window.print();
    return;
  }
  ...
  let y = 18;
  doc.setFontSize(18);
  doc.text("InvestSmart User Report", 14, y);
  y += 14;
  doc.setFontSize(11);
  doc.text(`User: ${userState.user.full_name} ${userState.user.surname}`, 14, y);
  y += 8;
  doc.text(`Net salary: ${money(finance.net_salary)}`, 14, y);
  y += 8;
  doc.text(`Current savings: ${money(finance.current_savings)}`, 14, y);
  y += 8;
  doc.text(`Projected total: ${money(projectedPortfolioValue)}`, 14, y);
  y += 8;
  doc.text(`Expected monthly growth: ${money(projectedMonthlyGrowthTotal)}`, 14, y);
  y += 12;
  doc.text("Goals:", 14, y);
  y += 8;
  (goals.length ? goals : ["No goals saved yet."]).forEach((goal) => {
    doc.text(`- ${goal}`, 20, y);
    y += 8;
  });
  y += 4;
  doc.text("Plans:", 14, y);
  y += 8;
  userState.plans.forEach((plan) => {
    if (y > 270) {
      doc.addPage();
      y = 18;
    }
    doc.text(`${plan.user_plan_name} | ${plan.bank_name || "N/A"} | ${money(projectedTotalValue(plan))}`, 14, y);
    y += 8;
    doc.text(`Goal: ${plan.investment_goal || "N/A"} | Monthly growth: ${money(expectedMonthlyGrowth(plan))}`, 20, y);
    y += 10;
  });
  doc.save("investsmart-user-report.pdf");
}
