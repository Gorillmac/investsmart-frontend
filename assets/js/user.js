...
function planTable(editable) {
  ...
  return `<div class="table-wrap"><div class="table-tools"><input data-search-table placeholder="Search plan, bank, goal, or risk"></div><table><thead><tr><th>${planSortHeader("user_plan_name", "Plan Name")}</th><th>${planSortHeader("investment_goal", "Goal")}</th><th>${planSortHeader("investment_amount", "Amount")}</th><th>${planSortHeader("projected_total", "Projected Total")}</th><th>Monthly Growth</th><th>${planSortHeader("risk", "Risk")}</th><th>${planSortHeader("horizon", "Horizon")}</th><th>${planSortHeader("liquidity", "Liquidity")}</th><th>Monthly</th><th>${planSortHeader("bank_name", "Bank")}</th><th>Actions</th></tr></thead><tbody>${plans.map((plan) => `<tr><td>${escapeHtml(plan.user_plan_name)}</td><td>${escapeHtml(plan.investment_goal || "N/A")}</td><td>${money(plan.investment_amount)}</td><td>${money(projectedTotalValue(plan))}</td><td>${money(expectedMonthlyGrowth(plan))}</td><td><span class="pill risk-${String(plan.risk).toLowerCase()}">${escapeHtml(plan.risk)}</span></td><td>${escapeHtml(plan.horizon)}-term</td><td><span class="pill liquidity-${String(plan.liquidity).toLowerCase()}">${escapeHtml(plan.liquidity)}</span></td><td>${Number(plan.monthly_contribution) ? `Yes (${money(plan.monthly_amount)})` : "No"}</td><td>${escapeHtml(plan.bank_name || "N/A")}<br><span class="muted">${escapeHtml(plan.bank_contact || "")}</span></td><td class="actions"><button class="secondary" data-view-plan="${plan.id}">View</button>${editable ? `<button class="secondary" data-edit-plan="${plan.id}">Edit</button><button class="danger" data-delete-plan="${plan.id}">Delete</button>` : ""}</td></tr>`).join("") || `<tr><td colspan="11" class="table-empty">No plans saved yet.</td></tr>`}</tbody></table></div>`;
}
